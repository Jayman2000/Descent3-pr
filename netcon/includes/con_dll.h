...
int StartMultiplayerGameMenu() {
  ...
  for (auto netgames_directory : DLLcf_LocateMultiplePaths("netgames")) {
  ...
  for (auto base_directory : *DLLBase_directories) {
    search_paths.push_back({base_directory / "data" / "levels", std::regex(".+\\.msn")});
    search_paths.push_back({base_directory / "missions", std::regex(".+\\.mn3")});
  };
