  ...
  for (i = 0; i < MAX_NET_PLAYERS; i++) {
    ...
    for (j = 0; j < MAX_NET_PLAYERS; j++) {
      ...
      if (stricmp(DLLMPlayers[i].tracker_id, DLLMPlayers[j].tracker_id) == 0) {
        ...
        DLLddio_MakePath(errfilepath, DLLGetWritableBaseDirectory(), "pxo.err", NULL);
...
int MTVersionCheck() {
  ...
  DLLddio_MakePath(fulldllpath, DLLGetWritableBaseDirectory(), "mtav.dll", NULL);
  if (!DLLmod_LoadModule(&MTAVDLLHandle, fulldllpath, MODF_LAZY)) {
    ...
    DLLddio_MakePath(szolddll, DLLGetWritableBaseDirectory(), "mtav.dll", NULL);
    DLLddio_MakePath(szbakdll, DLLGetWritableBaseDirectory(), "mtav.bak", NULL);
  ...
  if (rcode == 1) {
    if (MTUpdateURL[0]) {
      ...
      sprintf(sznewdll, "%s\\newmtav.dll", DLLGetWritableBaseDirectory());
      DLLddio_MakePath(sznewdll, DLLGetWritableBaseDirectory(), "newmtav.dll", NULL);
